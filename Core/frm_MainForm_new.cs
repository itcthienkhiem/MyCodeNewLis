...
namespace CIS.CoreApp
{
    public partial class frm_MainForm_new : Form
    {
        ...
        private Logger log;
        private bool fored2Close = false;
        private bool Click2Update = false;
        ...
        public static bool GetLicenseKey()
        {
            try
            {
                ...
                {
                    strEncryptedKey = Interaction.GetSetting(EnDec.Reg_Folder, EnDec.Reg_Section, EnDec.cstr_LICENSE_KEY,
                        String.Empty);
        ...
        private string AppName = Application.StartupPath + @"\HIS.exe";
        ...
        private void LoadFormMain()
        {
            try
            {
                ...
                //else
                //{
                //    Utility.ShowMsg("Hê thống không thể kích hoat được vì chưa đăng ký bản quyền, Bạn hãy liên hệ với IT để được trợ giúp ", "Thông báo");
                //}
                ...
                if (globalVariables.LoginSuceess)
                {
                    ...
                    Application.DoEvents();
                    SetInfor();
                    lbltieudegiaodien.Text = PropertyLib._AppProperties.TieuDe;
                    LoadMenu();
                }
            }
        ...
        private void SetInfor()
        {
            try
            {
                lblHospital.Text = globalVariables.Branch_Name + " Tuyến: " +
                                   THU_VIEN_CHUNG.Laygiatrithamsohethong("BHYT_TUYEN", "NONE", true);
                ...
                //Text = globalVariables.FORMTITLE;
                Text = PropertyLib._AppProperties.TieuDe;
            }
            catch (Exception ex)
            {
        ...
        private bool isSingleFunction = false;
        ...
        private List<string> lstTab = new List<string>();
        private Dictionary<string, UIMdiChildTab> dic = new Dictionary<string, UIMdiChildTab>();

...
Public Class frmNewPatientReg_VietDuc
    Public dtDepartment As DataTable
    Public dtObjectType As DataTable
    ...
    Public AutoLoadRegForm As Boolean = False
    ...
    Private Sub ReDesignForm()
        Try
            Select Case SysPara.BarcodeType.ToUpper()
                Case "VIETDUC"
                    Me.Size = New Size(Me.Size.Width, Me.Size.Height - cboLot.Size.Height - 5)
            End Select
    ...
    Private Sub LoadData()
        Try
            If Not mv_DR Is Nothing Then
                ...
                If (mv_DR(LPatientInfo.Columns.Dateupdate) <> Nothing) Then
                    dtmDate.Value = mv_DR(LPatientInfo.Columns.Dateupdate)
                End If
    ...
    Private Sub RegNewPatient()

        Try
            If Not bCheckData() Then Return
            Dim clsReg As New clsRegistry
            clsReg.SaveReg(2, "DVC_COMPANY", "APP_DVC", "PRJBUSSINESS_OBJECTTYPE", cboObject.SelectedIndex)
            clsReg.SaveReg(2, "DVC_COMPANY", "APP_DVC", "PRJBUSSINESS_CLOSEAFTERINSERT", _
                           IIf(chkCloseAfterInsert.Checked, 1, 0))
            clsReg.SaveReg(2, "DVC_COMPANY", "APP_DVC", "PRJBUSSINESS_DOB", IIf(chkDOB.Checked, 1, 0))
            Select Case m_iAction
                Case action.Insert 'Add Patient
                    If AddPatient() Then
                        'mv_bCancel = False
                        InsertIntoDataTableAndDataGrid()
                        If chkCloseAfterInsert.Checked Or AutoLoadRegForm Then
                            Close()
                        Else
                            'txtPID.Text = Utility.GetYYYYMMDDHHMMSS(Utilities.GetSysDate())
                            txtPatient_ID.Text = -1
                            txtPID.Text = Utility.GetYYYYMMDDHHMMSS(BusinessHelper.GetSysDateTime())
    ...
    Private Function AddPatient() As Boolean
        Try
            ...
            obj.IsNew = True
            obj.Save()
            txtPatient_ID.Text = obj.PatientId
    ...
    Private Sub frmNewPatientReg_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles MyBase.KeyDown
        If e.Control And e.KeyCode = Keys.S Then btnSave.PerformClick()
        If e.Alt And e.KeyCode = Keys.S Then btnSaveAndReg.PerformClick()
    ...
    Sub InsertIntoDataTableAndDataGrid()
        Try
            ...
            'grdList.MoveFirst()
            'Dim rowPatient = (From row In grdList.GetDataRows() Where row.Cells("Patient_ID").Value = txtPatient_ID.Text Select row).First()
            Dim rowPatient As GridEXRow = (From row In grdList.GetDataRows() Where row.Cells("Patient_ID").Value = txtPatient_ID.Text Select row).First()
            If rowPatient IsNot Nothing Then
                grdList.MoveTo(rowPatient)
            End If
        Catch ex As Exception
            ...
            If chkDOB.Checked Then
                record = _
                    New Update(LPatientInfo.Schema.TableName).Set(LPatientInfo.Columns.PatientName).EqualTo(txtPName.Text). _
                        ...
                        Set(LPatientInfo.Columns.Dateupdate).EqualTo(dtmDate.Value). _
    ...
    Private Sub btnSaveAndReg_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSaveAndReg.Click
    ...
    Private Sub txtDepartment_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtDepartment.TextChanged
        Try
            cboDepartment.SelectedValue = CInt(txtDepartment.Text)
        Catch ex As Exception

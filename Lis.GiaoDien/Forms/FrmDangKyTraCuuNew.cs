        ...
        private void PrintPageHandler(object sender, PrintPageEventArgs e)
        {
            ...
            try
            {
                ...
                for (i = 1; i <= Convert.ToInt32(_myProperties.SoTemp); i++)
                {
                    ...
                    e.Graphics.DrawImage(Barcode1.Image(_myProperties.Width, _myProperties.Height), marginLeft, marginTop, PrintAreaWidth,
                                         PrintAreaHeight);
       ...
       public void Printbarcode()
        {
            ...
            try
            {
               ...
               // GridEXRow[] rows = grdPatients.GetCheckedRows();
                      if (_myProperties.CauHinhInBarcode)
                {
                ...
                else
                {
                    ...
                    frmPrintBarcode a = new frmPrintBarcode();
                    ...
                    for (i = Convert.ToInt32(sobatdau); i <= Convert.ToInt32(Soketthuc); i++)
                    {
                        ...
                        if (!chk4so)
                        {
                            str += a.dtpNgay.Value.ToString("yyyy").Substring(2, 2);
                            str += a.dtpNgay.Value.ToString("MM");
        ...
        {
            if (_myProperties.CauHinhInBarcode == true)
            {
                Printbarcode();
            }
            else
        ...
        private void cmd_InPhieu_XetNghiem_TongHop_Click(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               IsQuick = false;
                ...
                {
                    INPHIEU_XETNGHIEM(true,  dtpDatePrint.Value);
                }
            }
            catch (Exception ex)
            {
        ...
        private void ProcessData(ref DataTable DT)
        {
            try
            {
                ...
                Utility.AddColumToDataTable(ref DT, "PIDImages", typeof(byte[]));
                ...
                foreach (DataRow dr in DT.Rows)
                {
                    ...
                    dr["PIDImages"] =
                        Utility.GenerateBarCode(
                            BarcodeInfo.CreateNewBarcode(Utility.sDbnull(dr["PID"], "0000000000")));
                    dr["logo"] = logoBytes;
        ...
        private void INPHIEU_XETNGHIEM(bool IsQuick,DateTime NgayIn)
        {
            ...
            if (crpt.FilePath != null &&crpt.FilePath!="")
            {
                ...
                try
                {
                    ...
                    Utility.SetParameterValue(crpt, "sPhone", globalVariables.Branch_Phone);
                    Utility.SetParameterValue(crpt, "sCurrentDate", Date(NgayIn));
                    Utility.SetParameterValue(crpt, "sTitleReport", titleInPhieuKQ);
        ...
        private void cmdValidate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (_myProperties.ConfirmResult == false)
                {
        ...
        {
            INPHIEU_XETNGHIEM(false, dtpDatePrint.Value);
        }

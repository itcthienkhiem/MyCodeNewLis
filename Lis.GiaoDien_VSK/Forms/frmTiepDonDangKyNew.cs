...
{
    public partial class frmTiepDonDangKyNew : Form
    {
        #region Attributes
        ...
        List<clsTenPhongBan> TenPhongBan = new List<clsTenPhongBan>();
        ...
        void LaydmPhongBan()
        {
            var loz = from p in m_dtDepartment.AsEnumerable()
                select new clsTenPhongBan()
                {
                    TenPhongBan = p.Field<string>("sName").ToString()
                };
            TenPhongBan = loz.ToList();
            txtDepartment.PhongBan(TenPhongBan);
        ...
        private void cboDepartment_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var query = from loz in m_dtDepartment.AsEnumerable()
                            ...
                            select loz[LDepartment.Columns.SName];
        ...
        private void txtDepartment_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    rowFilter = string.Format("{0}='{1}'", LDepartment.Columns.SName, txtDepartment.Text);
                }
        ...
        private void cmd_InPhieu_ChiDinh_Click(object sender, EventArgs e)
        {
            try
            {
                INPHIEU_XETNGHIEM(true, DateTime.Now);
            }
            catch (Exception)
            {
